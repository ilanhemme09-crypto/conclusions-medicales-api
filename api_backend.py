...
def traiter_texte_complet(texte: str):
    """Split sur RET, double points (..) ET sur >, traite marqueurs inline"""
    if not texte:
        return []
    
    try:
        # D'abord normaliser : remplacer ". ." par ".."
        texte_normalise = texte.replace('. .', '..')
        
        # Split sur RET, double points (..) ET sur >
        phrases = []
        phrase_courante = ""
        
        i = 0
        while i < len(texte_normalise):
            # Vérifier RET (3 caractères)
            if i + 2 < len(texte_normalise) and texte_normalise[i:i+3] == 'RET':
                # RET trouvé = fin de phrase
                if phrase_courante.strip():
                    phrases.append(phrase_courante.strip())
                phrase_courante = ""
                # Sauter RET (3 caractères)
                i += 3
                # Sauter aussi les espaces qui suivent
                while i < len(texte_normalise) and texte_normalise[i] == ' ':
                    i += 1
                continue
            
            char = texte_normalise[i]
            
            # Vérifier si on a un double point (..)
            if char == '.' and i + 1 < len(texte_normalise) and texte_normalise[i + 1] == '.':
...
@app.get("/")
async def root():
    return {"service": "API v4.3.6", "status": "running", "features": ["Apostrophes normalization", "RET + .. + >", "Alt debug"]}
