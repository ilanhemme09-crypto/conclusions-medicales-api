...
# -*- coding: utf-8 -*-
"""
API Backend v4.2.0 - Nouvelles fonctionnalités
- $$ commentaires gris $$
- ## texte suppressible ##
"""
...
app = FastAPI(title="API Conclusions v4.2.0")
...
def extraire_propositions(texte: str):
    """Extrait les propositions XXXX PROPOSITION...FINI"""
    propositions_map = {}
    try:
...
def extraire_alternatives(texte: str):
    """Extrait les alternatives @@ Info1 ; Info2 ; Info3 @@"""
    alternatives_map = {}
    try:
        pattern = r'@@\s*([^@]+?)\s*@@'
        for match in re.finditer(pattern, texte, re.DOTALL):
            position = match.start()
            alternatives_str = match.group(1).strip()
            alternatives = [a.strip() for a in alternatives_str.split(';') if a.strip()]
            if alternatives:
                alternatives_map[position] = alternatives
        return texte, alternatives_map
    except Exception as e:
        print(f"Erreur extraire_alternatives: {e}")
        return texte, {}

def extraire_commentaires(texte: str):
    """Extrait les commentaires {{ texte }}"""
    commentaires_map = {}
    try:
        pattern = r'\{\{\s*([^}]+?)\s*\}\}'
        for match in re.finditer(pattern, texte, re.DOTALL):
            ...
            commentaires_map[position] = commentaire
        return texte, commentaires_map
    except Exception as e:
        print(f"Erreur extraire_commentaires: {e}")
        return texte, {}

def extraire_deletables(texte: str):
    """Extrait les textes suppressibles [[ texte ]]"""
    deletables_map = {}
    try:
        pattern = r'\[\[\s*([^\]]+?)\s*\]\]'
        for match in re.finditer(pattern, texte, re.DOTALL):
            position = match.start()
            deletable = match.group(1).strip()
            deletables_map[position] = deletable
        return texte, deletables_map
    except Exception as e:
        print(f"Erreur extraire_deletables: {e}")
        return texte, {}

def extraire_lignes(texte: str):
    """Extrait les lignes sur les \n SANS ajouter de points"""
    if not texte:
        return []
    try:
        lignes = []
        ...
        for partie in parties:
            partie = partie.strip()
            if partie:  # Ne pas ajouter de lignes vides
...
def traiter_texte_complet(texte: str):
    """Traite XXXX, @@, $$, ## dans le texte"""
    if not texte:
        return []
    try:
        # 1. Extraire toutes les structures
        texte_propre, propositions_map = extraire_propositions(texte)
        texte_propre, alternatives_map = extraire_alternatives(texte_propre)
        texte_propre, commentaires_map = extraire_commentaires(texte_propre)
        texte_propre, deletables_map = extraire_deletables(texte_propre)
        
        # 2. Trouver toutes les positions des marqueurs
        xxxx_matches = [(m.start(), 'XXXX', 4) for m in re.finditer(r'XXXX', texte_propre, re.IGNORECASE)]
        alt_matches = [(m.start(), 'ALT', len(m.group(0)), m.group(1).strip()) for m in re.finditer(r'@@\s*([^@]+?)\s*@@', texte_propre, re.DOTALL)]
        comment_matches = [(m.start(), 'COMMENT', len(m.group(0)), m.group(1).strip()) for m in re.finditer(r'\{\{\s*([^}]+?)\s*\}\}', texte_propre, re.DOTALL)]
        deletable_matches = [(m.start(), 'DELETE', len(m.group(0)), m.group(1).strip()) for m in re.finditer(r'\[\[\s*([^\]]+?)\s*\]\]', texte_propre, re.DOTALL)]
        
        # Combiner et trier par position
        all_markers = xxxx_matches + alt_matches + comment_matches + deletable_matches
        ...
        for marker in all_markers:
            ...
            texte_avant = texte_propre[derniere_pos:pos].strip()
            if texte_avant:
            ...
            if marker_type == 'XXXX':
                ...
                derniere_pos = pos + marker_len
                
            elif marker_type == 'ALT':
                alt_content = marker[3]
                alts = None
                for alt_pos, alternatives in alternatives_map.items():
                    if abs(pos - alt_pos) < 50:
                        alts = {"alternatives": alternatives}
                        break
                texte_affiche = alts["alternatives"][0] if alts else alt_content
                lignes_finales.append({
                    "texte": texte_affiche,
                    "proposition": None,
                    "alternative": alts,
            ...
            elif marker_type == 'COMMENT':
                comment_content = marker[3]
                lignes_finales.append({
                    "texte": comment_content,
                    "proposition": None,
                    "alternative": None,
                    "commentaire": True,
                    "deletable": False
            ...
            elif marker_type == 'DELETE':
                delete_content = marker[3]
                lignes_finales.append({
                    "texte": delete_content,
                    "proposition": None,
                    "alternative": None,
                    "commentaire": None,
                    "deletable": True
        ...
        # Si aucun marqueur, juste extraire les lignes
        if not all_markers:
...
@app.get("/")
async def root():
    return {"service": "API v4.2.0", "status": "running", "features": ["@@ alternatives", "$$ comments", "## deletables"]}
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 API v4.2.0 - $$ comments + ## deletables")
