...
@app.get("/")
async def root():
    return {"service": "API v4.4.5-STABLE", "status": "running", "endpoints": ["/health", "/categories", "/motifs/{categorie}", "/fusion"]}

@app.get("/health")
async def health():
    return {"status": "healthy", "version": "4.4.5"}
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            resp = await client.get(
                f"{SUPABASE_URL}/rest/v1/traumatologie?select=categorie&order=categorie",
                headers=headers
            )
            
            if resp.status_code == 200:
                data = resp.json()
                categories = sorted(list(set([item['categorie'] for item in data if item.get('categorie')])))
...
async def get_motifs(categorie: str):
    async with httpx.AsyncClient() as client:
        try:
            ...
            resp = await client.get(
                f"{SUPABASE_URL}/rest/v1/traumatologie?categorie=eq.{categorie}&select=id,nom_motif&order=nom_motif",
                headers=headers
            )
