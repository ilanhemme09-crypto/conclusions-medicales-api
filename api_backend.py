...
# -*- coding: utf-8 -*-
"""
API Backend v4.0.0 ULTRA-SIMPLE
"""
...
def traiter_texte_avec_xxxx(texte: str):
    """Retourne une liste de dicts au lieu d'objets Pydantic"""
    if not texte:
        return []
    try:
        ...
        for idx, (xxxx_pos, props) in enumerate(xxxx_avec_props):
            ...
            if texte_avant:
                ...
                for ligne in lignes_texte:
                    lignes_finales.append({"texte": ligne, "proposition": None})
            lignes_finales.append({"texte": "XXXX", "proposition": props})
        ...
        return [{"texte": l, "proposition": None} for l in lignes_simple]
...
@app.get("/")
async def root():
    return {"service": "API v4.0.0 ULTRA-SIMPLE", "status": "running"}
