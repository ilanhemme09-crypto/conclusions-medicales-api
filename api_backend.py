...
def traiter_ligne_avec_marqueurs(ligne: str):
    ...
    pattern_alt = r'@@\s*([^@]+?)\s*@@'  # Alternatives avec /
...
def traiter_texte_complet(texte: str):
    """Split UNIQUEMENT sur points, traite marqueurs inline"""
    if not texte:
        return []
    
    try:
        # Split sur les points UNIQUEMENT
        # Utiliser un split qui préserve les autres caractères
        phrases = []
        phrase_courante = ""
        
        for char in texte:
            if char == '.':
                # On a trouvé un point, terminer la phrase
...
@app.get("/")
async def root():
    return {"service": "API v4.3.1", "status": "running", "features": ["HDM 4 sections", "/ separator", "split on dots only"]}
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 API v4.3.1 - Slash separator + Split dots only")
