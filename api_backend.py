...
# -*- coding: utf-8 -*-
"""
API Backend v4.6.0 - FUSION COMPLETE
- Tous les modules fusionnent maintenant tous les motifs
- Fix KeyError 'texte' dans ordonnances
- Structure lignes.elements groupés
"""
...
app = FastAPI(title="API Conclusions v4.6.0")
...
def traiter_texte_complet(texte: str):
    """Split sur double points (..) ET sur >, traite marqueurs inline.
    Retourne une liste de lignes, chaque ligne contenant ses éléments groupés."""
    if not texte:
        return []
    
    try:
        # Split sur double points (..) ET sur >
        phrases = []
        phrase_courante = ""
        ...
        while i < len(texte):
            ...
            # Vérifier si on a un double point (..)
            if char == '.' and i + 1 < len(texte) and texte[i + 1] == '.':
                # Double point trouvé = fin de phrase
                if phrase_courante.strip():
                    phrases.append(phrase_courante.strip())
                phrase_courante = ""
                # Sauter les deux points
                i += 2
            elif char == '>':
                # > trouvé = nouvelle ligne
                if phrase_courante.strip():
                    phrases.append(phrase_courante.strip())
                phrase_courante = ""
                # Sauter le > lui-même
        ...
        for phrase in phrases:
            ...
            if elements:
                # Chaque phrase devient une ligne avec ses éléments groupés
...
def supprimer_doublons(lignes):
    """Supprime les lignes en double. Chaque ligne a une structure {elements: [...]}"""
    try:
        vues = set()
        resultat = []
        for ligne in lignes:
            # Extraire le texte de tous les éléments de la ligne
            if "elements" in ligne:
                texte_complet = ' '.join(elem.get("texte", "") for elem in ligne["elements"])
            else:
                texte_complet = ligne.get("texte", "")
            
            ligne_norm = ' '.join(texte_complet.lower().split())
            if ligne_norm and ligne_norm not in vues:
                vues.add(ligne_norm)
...
@app.get("/")
async def root():
    return {"service": "API v4.3.3", "status": "running", "features": ["HDM titres outside", "split on ..", "conduite editable"]}
    ...
    try:
        ...
        if request.mode in ["conclusion", "examen_conclusion"]:
            for module_type, config in MODULES_CONCLUSION.items():
                ...
                # Fusionner TOUS les motifs pour TOUS les modules
                for motif in tous_les_motifs:
                    if motif.get(module_type):
                        texte = motif[module_type]
                        texte_propre, bulles = parse_bulles(texte)
        ...
        for motif in tous_les_motifs:
            ...
            for titre_ordo, contenu_ordo in ordos.items():
                ...
                # Extraire le texte de la première ligne pour la clé de déduplication
                premier_texte = ""
                if lignes and lignes[0].get("elements"):
                    premier_texte = ''.join(e.get("texte", "") for e in lignes[0]["elements"])[:30]
                
                cle = f"{titre_ordo}_{premier_texte}"
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 API v4.6.0 - Fusion complète tous modules + Structure lignes.elements")
