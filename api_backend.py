...
def traiter_ligne_avec_marqueurs(ligne: str):
    ...
    for m in re.finditer(pattern_xxxx, ligne, re.IGNORECASE):
        ...
        print(f"[PARSE] XXXX à {m.start()}-{m.end()}, props: {suggestions}")
...
@app.get("/")
async def root():
    return {"service": "API v4.4.2", "status": "running", "features": ["Debug logs enabled", "Full parsing", "Inline rendering"]}
