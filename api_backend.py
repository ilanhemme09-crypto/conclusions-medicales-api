...
def traiter_texte_complet(texte: str):
    """Split UNIQUEMENT sur RET, retire RET du texte, traite marqueurs inline"""
    if not texte:
        return []
    
    try:
        # Split UNIQUEMENT sur RET
        # RET ne doit PAS apparaître dans le texte final
        phrases = []
        phrase_courante = ""
        ...
        for phrase in phrases:
            ...
            # Traiter les marqueurs dans cette phrase - retourne liste d'éléments
            elements = traiter_ligne_avec_marqueurs(phrase)
            
            # IMPORTANT: Tous les éléments de cette phrase vont sur UNE SEULE ligne
            # On ne les sépare PAS en plusieurs lignes
            lignes_finales.append(elements)
