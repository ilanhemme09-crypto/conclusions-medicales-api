...
# -*- coding: utf-8 -*-
"""
API Backend v4.0.0 FIXED - Retours corrects des lignes
"""
...
app = FastAPI(title="API Conclusions v4.0.0 FIXED")
...
class LigneAvecProposition(BaseModel):
    ...
    class Config:
        # Force la sérialisation explicite
        json_schema_extra = {
            "example": {
...
class Module(BaseModel):
    ...
    class Config:
        # S'assurer que la sérialisation est correcte
        json_encoders = {
            LigneAvecProposition: lambda v: {"texte": v.texte, "proposition": v.proposition.dict() if v.proposition else None}
        }
...
def traiter_texte_avec_xxxx(texte: str) -> List[LigneAvecProposition]:
    if not texte:
        return []
    try:
        ...
        print(f"✅ traiter_texte_avec_xxxx: {len(lignes_finales)} lignes créées")
        for l in lignes_finales[:3]:
            print(f"  - Type: {type(l)}, texte: {l.texte[:50] if l.texte else 'vide'}")
...
@app.get("/")
async def root():
    return {"service": "API v4.0.0 FIXED", "status": "running", "features": ["examen", "conclusion", "examen_conclusion"]}
    ...
    try:
        ...
        if request.mode in ["conclusion", "examen_conclusion"]:
            for module_type, config in MODULES_CONCLUSION.items():
                ...
                if lignes_uniques:
                    print(f"Module {module_type}: {len(lignes_uniques)} lignes uniques")
                    modules_finaux.append(Module(
                        type=module_type,
                        titre=config['titre'],
                        icon=config['icon'],
        ...
        # Créer la réponse avec dict() explicite pour forcer la sérialisation
        ...
        # Convertir en dict avant de retourner
        response_dict = response.dict()
        
        print(f"✅ Réponse créée: {len(response_dict['modules'])} modules")
        if response_dict['modules']:
            premier_module = response_dict['modules'][0]
            print(f"  Premier module: {premier_module['titre']}")
            print(f"  Type lignes: {type(premier_module['lignes'])}")
            if premier_module['lignes']:
                print(f"  Première ligne type: {type(premier_module['lignes'][0])}")
                print(f"  Première ligne: {premier_module['lignes'][0]}")
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 API v4.0.0 FIXED - Sérialisation correcte")
