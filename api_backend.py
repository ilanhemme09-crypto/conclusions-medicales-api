...
# -*- coding: utf-8 -*-
"""
API Backend - Conclusions Médicales v3.0.1
CORRECTION : Gestion d'erreurs robuste
"""
...
app = FastAPI(title="API Conclusions v3.0.1")
...
def extraire_propositions(texte: str):
    """
    Extrait les XXXX avec leurs propositions
    Format: XXXX PROPOSITION : prop1 ; prop2 ; prop3 FINI
    
    Retourne: liste de tuples (position, propositions ou None)
    """
    propositions_map = {}
    
    try:
        # Pattern pour XXXX avec PROPOSITION
        ...
        # Nettoyer le texte des PROPOSITION...FINI
...
def traiter_texte_avec_xxxx(texte: str):
    """
    Traite le texte en gérant XXXX et propositions
    """
    if not texte:
        return []
    
    try:
        # 1. Extraire propositions AVANT de parser le reste
        texte_propre, propositions_map = extraire_propositions(texte)
        
        # 2. Trouver toutes les positions de XXXX
        xxxx_positions = [m.start() for m in re.finditer(r'XXXX', texte_propre, re.IGNORECASE)]
        
        # 3. Associer propositions aux XXXX
        xxxx_avec_props = []
        for pos in xxxx_positions:
            # Chercher si cette position a des propositions
            props = None
            for prop_pos, suggestions in propositions_map.items():
                if abs(pos - prop_pos) < 50:  # Tolérance de 50 caractères
                    props = Proposition(suggestions=suggestions)
                    break
            xxxx_avec_props.append((pos, props))
        
        # 4. Découper le texte en segments autour des XXXX
        lignes_finales = []
        derniere_pos = 0
        ...
        for idx, (xxxx_pos, props) in enumerate(xxxx_avec_props):
            ...
            derniere_pos = xxxx_pos + 4  # Longueur de "XXXX"
        
        # Texte après le dernier XXXX
        ...
        # Si pas de XXXX du tout, juste extraire les lignes
...
def supprimer_doublons(lignes: List[LigneAvecProposition]) -> List[LigneAvecProposition]:
    """Supprime les doublons en comparant le texte"""
    try:
        vues = set()
        resultat = []
...
@app.get("/")
async def root():
    return {"service": "API Conclusions v3.0.1", "version": "3.0.1", "status": "running"}
...
async def fusion_motifs(request: FusionRequest):
    try:
        print(f"=== DÉBUT FUSION ===")
        print(f"Table: {request.table_principale}, Motif: {request.motif_principal_id}")
        ...
        print(f"Nombre motifs: {len(tous_les_motifs)}")
        ...
        for module_type, config in MODULES_CONFIG.items():
            try:
                ...
                if lignes_uniques:
                    ...
                    print(f"Module {module_type}: {len(lignes_uniques)} lignes")
                    
            except Exception as e:
                print(f"Erreur module {module_type}: {e}")
                traceback.print_exc()
        ...
        try:
            ...
            print(f"Erreur codes CIM: {e}")
        
        print(f"=== FIN FUSION ===")
        print(f"Modules: {len(modules_finaux)}, Ordonnances: {len(ordonnances_finales)}, Codes: {len(codes_finaux)}")
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 API v3.0.1 - Version corrigée")
