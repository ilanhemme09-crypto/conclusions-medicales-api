...
def traiter_ligne_avec_marqueurs(ligne: str):
    """Parse TOUS les marqueurs et retourne liste d'éléments inline"""
    if not ligne:
        return []
    
    print(f"\n[PARSE] ===== LIGNE =====")
    print(f"[PARSE] Input: {repr(ligne)}")
    ...
    # Trouver TOUS les marqueurs avec positions
    marqueurs = []
    
    # XXXX
    xxxx_matches = list(re.finditer(pattern_xxxx, ligne, re.IGNORECASE))
    print(f"[PARSE] Recherche XXXX: {len(xxxx_matches)} trouvés")
    for i, m in enumerate(xxxx_matches):
        props = m.group(1)
        suggestions = [s.strip() for s in props.split(';')] if props else []
        marqueurs.append((m.start(), m.end(), 'XXXX', suggestions))
        print(f"[PARSE] XXXX {i+1}: position {m.start()}-{m.end()}, texte='{ligne[m.start():m.end()]}', props: {suggestions}")
    ...
    # Si aucun marqueur
    if not marqueurs:
        print(f"[PARSE] Aucun marqueur, retour texte brut")
        return [{
            "texte": ligne,
            "proposition": None,
            "alternative": None,
    ...
    print(f"[PARSE] Total éléments: {len(elements)}")
    print(f"[PARSE] ===== FIN =====\n")
    
    return elements

def traiter_texte_complet(texte: str):
    """Split sur RET, parse marqueurs, retourne liste d'éléments avec marqueurs de ligne"""
    if not texte:
        return []
    
    print(f"\n[TEXTE_COMPLET] ========== DÉBUT ==========")
    print(f"[TEXTE_COMPLET] Texte reçu: {repr(texte)}")
    print(f"[TEXTE_COMPLET] Longueur: {len(texte)}")
    ...
    try:
        ...
        print(f"[TEXTE_COMPLET] Phrases après split sur RET: {len(phrases)}")
        for i, p in enumerate(phrases):
            print(f"[TEXTE_COMPLET] Phrase {i}: {repr(p[:100] if len(p) > 100 else p)}")
        
        # Parser chaque phrase avec les marqueurs
        lignes_finales = []
        ...
        for idx, phrase in enumerate(phrases):
            ...
            # Parser les marqueurs dans cette phrase
            elements = traiter_ligne_avec_marqueurs(phrase)
            
            # Marquer le premier élément de chaque ligne (sauf la première)
...
@app.get("/")
async def root():
    return {"service": "API v4.4.3", "status": "running", "features": ["MASSIVE DEBUG LOGS", "Full parsing trace", "Text analysis"]}
